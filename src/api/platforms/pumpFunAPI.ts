...
const BASE_URL = 'https://api.pump.fun';

// API client for pump.fun
class PumpFunAPI {
  // Get trending tokens
  async getTrendingTokens(): Promise<Token[]> {
    try {
      ...
      console.error('Error fetching trending tokens from pump.fun:', error);
      return [];
    }
  }
  ...
  async getRecentTokens(): Promise<Token[]> {
    try {
      ...
      console.error('Error fetching recent tokens from pump.fun:', error);
      return [];
    }
  }
  ...
  async getTokenDetails(address: string): Promise<Token | null> {
    try {
      ...
      console.error(`Error fetching token details for ${address} from pump.fun:`, error);
      return null;
    }
  }
  ...
  async searchTokens(query: string): Promise<Token[]> {
    try {
      ...
      console.error(`Error searching tokens with query "${query}" from pump.fun:`, error);
      return [];
    }
  }
  ...
  async simulateTrade(
    ...
    try {
      ...
      console.error('Error simulating trade on pump.fun:', error);
      return {
        success: false,
        estimatedPrice: 0,
        totalValue: 0,
